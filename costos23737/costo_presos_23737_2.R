...
ley23737_sobre_total['costo_23737_actualizado'] <- c(7405181,	293025514,	1049198333,	1331698394,	1590981370,	1768591822,	2172346037,	2413024101,	2471304479,	2059110717,	2575693129,	2778888074,	3619223005,	3801600713,	3210741501)
...
anios_meses <- read_excel("data/anios-meses.xlsx") 
...
presupuesto_spb_2 <- read_excel("C:/Users/Usuario/Desktop/Lucas/1- pe/5-presos/DATA/presupuesto_spb_por_año.xlsx")
...
for(i in 1:nrow(metodo2_23737)){
  ...
  for(j in i:nrow(costo_preso_2)){
    presupuesto <- presupuesto*(1+costo_preso_2$inflacion[j])
  }
...
sum(metodo2_23737$costo_presos23737_mensual_actualizado) # 30647816161
sum(metodo2_agrupado$costo_presos23737_anual_actualizado) # 30647816161
...
sum(metodo3$costo_presos23737_mensual_promedio_actualizado) # 30605694854

(30605694854-30647816161)/30647816161 # = -0.001374366

metodo3_agrupado <- metodo3 %>% group_by(anio) %>% summarise(costo_metodo3 = sum(costo_presos23737_mensual_promedio_actualizado))

comparacion_metodos <- metodo3$anio %>% cbind(metodo3$mes) %>% cbind(metodo2_23737$costo_presos23737_mensual_actualizado) %>% 
  cbind(metodo3$costo_presos23737_mensual_actualizado) %>% cbind(metodo3$costo_presos23737_mensual_promedio_actualizado)
colnames(comparacion_metodos) <- c('anio','mes','metodo1_31dic','metodo2_lineal','metodo3_promedio')
View(comparacion_metodos)
###########################################################
# comparacion anual
...
###### hacer el metodo1 pero por mes

ley23737_sobre_total_copia2 <- ley23737_sobre_total_copia2 %>% mutate(presos_23737_relativo=presos_ley23737/presos_total)
colnames(ley23737_sobre_total_copia2) <- c('anio','presos_ley23737','presos_total','presos_23737_relativo')
ley23737_sobre_total_copia2 <- ley23737_sobre_total_copia2 %>% inner_join(presupuesto_spb,by='anio')
data_2019 <- c(2019,	4915,	45366,	0.108341,	23417065600, 2537028304	) # datos sacados de SERVICIO PENITENCIARIO BONAERENSE | UNIDAD DE GESTION Y COORDINACIÓN ESTADÍSTICA
ley23737_sobre_total_copia2 <- rbind(ley23737_sobre_total_copia2, data_2019)
